...
void entry_unions(const unsigned int buffer_size, int buffer[const])
{
    int i = 0;
    ...
    // the direct cast is a GNU extension and compcert does not support it
    // taking the address of i is not a workaround as it is UB
    // so I've further rewritten this
   /*union my_union u7;*/
    /*u7 = (union my_union)(i);*/
    /*buffer[i++] = u7.as_int;*/
    union my_union u7;
