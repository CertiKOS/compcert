...
val _Regexp : string -> pattern
(** text of an [Str] regexp *)
...
val parse_cmdline : (pattern * action) list -> string array -> unit
...
(** [longopt_int key fn] generates a pattern and an action for options of the
    form [key=<text>] and calls [fn] with the string argument *)

val longopt_int : string -> (int -> unit) -> pattern * action
(** [longopt_int key fn] generates a pattern and an action for options of the
    form [key=<n>] and calls [fn] with the integer argument *)
