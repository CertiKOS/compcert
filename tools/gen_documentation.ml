...
let copy_dir source_path target_path =
  let command =
    ...
    else Printf.sprintf "cp -a -R -f %s %s" source_path target_path
  in
  let _ = Sys.command command in
  ()

let to_directory platform =
  match platform with
  | "aarch64" -> "aarch64"
  | "arm" -> "arm"
  | "peaktop" -> "peaktop"
...
let copy_and_rename new_platform_name file_dir =
  ...
  (* rename platform *)
  Sys.rename
    (Filename.concat file_dir "TargetPlatformCopy")
    (Filename.concat file_dir new_platform_name)
...
let suffix_files_in_dirs directories suffix =
  List.map
    (fun dir ->
      let files_in_dir =
        List.map
          (fun file -> Filename.concat dir file)
          (Array.to_list (Sys.readdir dir))
      in
      let v_files =
        List.filter
          (fun f -> is_regular_file f && Filename.check_suffix f suffix)
          files_in_dir
      in
      (dir, v_files))
    directories
...
let dirs_and_files_of_suffix dir suffix =
  ...
  let content_prefixed =
    if dir = "." then content
    else List.map (fun l -> Filename.concat dir l) content
  in
  let directories =
    List.filter (fun f -> Sys.is_directory f) content_prefixed
  in
  (* add prefix dir *)
...
let collect_files_in_dirs dir =
  ...
  ( List.filter_map
      (fun (dir, files) -> if files = [] then None else Some dir)
      dir_file_pairs,
    List.concat_map snd dir_file_pairs )
...
let patch_file plat f =
  ...
  let patched_content =
    Str.global_replace regx ("." ^ plat ^ ".") read_content
  in
  (* Remove file *)
  Sys.remove f;
...
let docs_for_platform new_platform_name =
  ...
  let flocq_dirs, flocq_files =
    collect_files_in_dirs (Filename.concat ".." "flocq")
  in
  ...
  let globs =
    List.map
      (fun dir -> Filename.concat dir "*.glob")
      (List.concat [ plat_dirs; flocq_dirs; menhir_dirs ])
  in
  ...
  let command =
    Printf.sprintf "coq2html -d %s -base compcert -short-names %s %s" output_dir
      (String.concat " " globs) (String.concat " " files)
  in
...
let _ =
  ...
  else invalid_arg "expected only one file name argument"
