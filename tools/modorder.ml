...
type node = { deps : string list; result : string; mutable status : status }
...
let parse_dependencies depfile =
  ...
  let rec read_line () =
    ...
    if Str.string_match re_continuation_line l 0 then
  ...
  let enter_line l =
    if not (Str.string_match re_depend_line l 0) then
      eprintf "Warning: ill-formed dependency line '%s'\n" l
    else
      let lhs = Str.matched_group 1 l and rhs = Str.matched_group 2 l in
      let lhs = Str.split re_whitespace lhs
      and rhs = Str.split re_whitespace rhs in
      List.iter
        (fun lhs ->
          let node = { deps = rhs; result = lhs; status = Todo } in
...
let filename_suffix s =
  try
    ignore (Str.search_forward re_suffix s 0);
  ...
  let rec dsort target suff =
    ...
    | Some node -> (
        ...
        | Todo ->
            ...
            List.iter
              (fun dep -> if Filename.check_suffix dep suff then dsort dep suff)
              node.deps;
            printf "%s " target;
            node.status <- Done)
  in
