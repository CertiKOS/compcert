...
let match_args args =
  ...
  let subst s =
    incr n;
    sprintf "%s as zz%d" (Str.matched_group 1 s) !n
  in
...
let parenpats p = "(" ^ Str.global_replace re_comma ") (" p ^ ")"

(* "foo, bar, gee" -> "_ _ _" *)

let underscores_for s = Str.global_replace re_arg "_" (remove_commas s)
...
let matched_params params args =
  ...
  let filter_param s =
    if List.mem (Str.matched_group 1 s) arglist then Str.matched_string s
    else ""
  in
...
***)
  ...
  let transl_case s =
    ...
    if Array.length res = 0 then error filename lineno ("ill-formed case: " ^ s);
    let patlist = res.(1) and guard = res.(3) and rhs = res.(4) in
    let bv = boundvarspat patlist in
    if not (Str.string_match re_default_pat patlist 0) then (
      incr numcase;
      bprintf a "  | %s_case%d: forall %s, %s_cases %s\n" name !numcase bv name
        (parenpats patlist);
      if guard = "" then
      ...
      bprintf c "  end.\n\n"
  in
  List.iter transl_case (Str.split re_split_cases cases);
  ...
  try
    while true do
      ...
      if !in_nd then (
        ...
        Buffer.add_char b '\n';
        if Str.string_match re_end_nd_fun l 0 then (
          transl_ndfun f !line_start (Buffer.contents b);
          Buffer.clear b;
          in_nd := false))
      else if Str.string_match re_begin_nd_fun l 0 then (
        Buffer.clear b;
        Buffer.add_string b l;
        Buffer.add_char b '\n';
        in_nd := true;
