...
let is_two_address op args =
  if two_address_op op then
    match args with [] -> assert false | arg1 :: argl -> Some (arg1, argl)
  else None
...
let init_trace () =
  if !option_dalloctrace && !pp == stdout then
...
let constrain_reg v c = match c with None -> v | Some mr -> L (R mr)
...
let move v1 v2 k =
  if v1 = v2 then k
  else if XTL.is_stack_reg v1 then
    let t = new_temp (typeof v2) in
    Xmove (v1, t) :: Xmove (t, v2) :: k
  else if XTL.is_stack_reg v2 then
    let t = new_temp (typeof v1) in
    Xmove (v1, t) :: Xmove (t, v2) :: k
  else Xmove (v1, v2) :: k
  ...
  let rec expand srcs' dsts' rl ll =
    ...
    | r :: rl, Twolong (l1, l2) :: ll ->
        ...
        if Archi.splitlong then
          expand
            (V (r, Tint) :: V (twin_reg r, Tint) :: srcs')
            (L l1 :: L l2 :: dsts') rl ll
        else
...
let rec convert_builtin_arg tyenv = function
  | BA r ->
      let ty = tyenv r in
      if Archi.splitlong && ty = Tlong then
        BA_splitlong (BA (V (r, Tint)), BA (V (twin_reg r, Tint)))
      else BA (V (r, ty))
  | BA_int n -> BA_int n
  | BA_long n -> BA_long n
...
let convert_builtin_res tyenv = function
  | BR r ->
      let ty = tyenv r in
      if Archi.splitlong && ty = Tlong then
        BR_splitlong (BR (V (r, Tint)), BR (V (twin_reg r, Tint)))
      else BR (V (r, ty))
  | BR_none -> BR_none
...
let block_of_RTL_instr funsig tyenv = function
  ...
  | RTL.Iop (Omove, [ arg ], res, s) ->
      ...
      else [ Xmove (vreg tyenv arg, vreg tyenv res); Xbranch s ]
  ...
  | RTL.Iop (op, args, res, s) ->
      ...
      and res2 = constrain_reg res1 cres in
      let args3, res3 =
        match is_two_address op args2 with
        | None -> (args2, res2)
        | Some (arg, args2') ->
            if arg = res2 || not (List.mem res2 args2') then (args2, res2)
            else
              let t = new_temp (tyenv res) in
              (t :: args2', t)
      in
      movelist args1 args3
        (Xop (op, args3, res3) :: move res3 res1 [ Xbranch s ])
  | RTL.Iload (chunk, addr, args, dst, s) ->
      if Archi.splitlong && chunk = Mint64 then
      ...
      else [ Xload (chunk, addr, vregs tyenv args, vreg tyenv dst); Xbranch s ]
  | RTL.Istore (chunk, addr, args, src, s) ->
      ...
      else [ Xstore (chunk, addr, vregs tyenv args, vreg tyenv src); Xbranch s ]
  | RTL.Icall (sg, ros, args, res, s) ->
      ...
      parmove_regs2locs tyenv args args'
        (Xcall
           (sg, sum_left_map (vreg tyenv) ros, vlocpairs args', vlocpairs res')
        :: parmove_locs2regs tyenv res' [ res ] [ Xbranch s ])
  ...
  | RTL.Ibuiltin (ef, args, res, s) ->
      ...
      movelist
        (params_of_builtin_args args1)
        (params_of_builtin_args args2)
        (Xbuiltin (ef, args2, res2)
        :: movelist
             (params_of_builtin_res res2)
             (params_of_builtin_res res1)
             [ Xbranch s ])
  | RTL.Icond (cond, args, s1, s2) -> [ Xcond (cond, vregs tyenv args, s1, s2) ]
  | RTL.Ijumptable (arg, tbl) -> [ Xjumptable (vreg tyenv arg, tbl) ]
  | RTL.Ireturn None -> [ Xreturn [] ]
...
let live_before instr after =
  ...
  | Xmove (src, dst) | Xspill (src, dst) | Xreload (src, dst) ->
      if VSet.mem dst after || XTL.is_stack_reg src then
        VSet.add src (VSet.remove dst after)
      else after
  | Xparmove (srcs, dsts, itmp, ftmp) ->
      vset_addlist srcs (vset_removelist dsts after)
  | Xop (op, args, res) ->
      if VSet.mem res after then vset_addlist args (VSet.remove res after)
      else after
  | Xload (chunk, addr, args, dst) ->
      if VSet.mem dst after then vset_addlist args (VSet.remove dst after)
      else after
  | Xstore (chunk, addr, args, src) -> vset_addlist args (VSet.add src after)
  | Xcall (sg, ros, args, res) ->
      vset_addlist args (vset_addros ros (vset_removelist res after))
  | Xtailcall (sg, ros, args) -> vset_addlist args (vset_addros ros VSet.empty)
  | Xbuiltin (EF_debug _, args, res) -> vset_removeres res after
  | Xbuiltin (ef, args, res) -> vset_addargs args (vset_removeres res after)
  | Xbranch s -> after
  | Xcond (cond, args, s1, s2) -> List.fold_right VSet.add args after
  | Xjumptable (arg, tbl) -> VSet.add arg after
  | Xreturn args -> vset_addlist args VSet.empty
...
let liveness_analysis f =
  match
    Liveness_Solver.fixpoint f.fn_code successors_block (transfer_live f)
  with
  | None -> assert false
  | Some lv -> lv
...
let pair_block_live blk after =
  let rec pair_rec accu after = function
    ...
    | instr :: blk ->
        ...
        pair_rec ((instr, before) :: accu) before blk
  in
...
let rec dce_parmove srcs dsts after =
  ...
  | src1 :: srcs, dst1 :: dsts ->
      ...
      if VSet.mem dst1 after || XTL.is_stack_reg src1 then
        (src1 :: srcs', dst1 :: dsts')
      else (srcs', dsts')
  | _, _ -> assert false
...
let dce_instr instr after k =
  match instr with
  | Xmove (src, dst) ->
      if VSet.mem dst after || XTL.is_stack_reg src then instr :: k else k
  | Xparmove (srcs, dsts, itmp, ftmp) -> (
      match dce_parmove srcs dsts after with
      | [], [] -> k
      | [ src ], [ dst ] -> Xmove (src, dst) :: k
      | srcs', dsts' -> Xparmove (srcs', dsts', itmp, ftmp) :: k)
  | Xop (op, args, res) -> if VSet.mem res after then instr :: k else k
  | Xload (chunk, addr, args, dst) ->
      if VSet.mem dst after then instr :: k else k
...
let dead_code_elimination f liveness =
  {
    ...
    fn_code =
      PTree.map
        (fun pc blk -> snd (dce_block blk (PMap.get pc liveness)))
        f.fn_code;
  }
...
let spill_costs f =
  ...
  let charge amount uses v =
    ...
    | V (r, ty) ->
        ...
        (if st.cost < 0 then
           ...
           let c2 =
             if c1 >= 0 then c1 else max_int
             (* overflow *)
           in
           st.cost <- c2);
        st.usedefs <- st.usedefs + uses
  in

  let charge_list amount uses vl = List.iter (charge amount uses) vl in

  let charge_ros amount ros =
    match ros with Coq_inl v -> charge amount 1 v | Coq_inr id -> ()
  in
  ...
  let charge_instr = function
    | Xmove (src, dst) ->
        if XTL.is_stack_reg src then force_stack_allocation dst
        else if XTL.is_stack_reg dst then force_stack_allocation src
        else (
          charge 1 1 src;
          charge 1 1 dst)
    | Xreload (src, dst) ->
        charge 1 1 src;
        charge max_int 1 dst (* dest must not be spilled! *)
    | Xspill (src, dst) ->
        charge max_int 1 src;
        charge 1 1 dst (* source must not be spilled! *)
    | Xparmove (srcs, dsts, itmp, ftmp) ->
        charge_list 1 1 srcs;
        charge_list 1 1 dsts;
        charge max_int 0 itmp;
        charge max_int 0 ftmp (* temps must not be spilled *)
    | Xop (op, args, res) ->
    ...
    | Xstore (chunk, addr, args, src) ->
        charge_list 10 1 args;
        charge 10 1 src
    | Xcall (sg, vos, args, res) -> charge_ros 10 vos
    | Xtailcall (sg, vos, args) -> charge_ros 10 vos
    ...
    | Xbranch _ -> ()
    | Xcond (cond, args, _, _) -> charge_list 10 1 args
    | Xjumptable (arg, _) -> charge 10 1 arg
    | Xreturn optarg -> ()
  in

  let charge_block blk = List.iter charge_instr blk in

  PTree.fold1 (fun () blk -> charge_block blk) f.fn_code ();
  if !option_dalloctrace then (
    fprintf !pp "------------------ Unspillable variables --------------\n\n";
    PTree.fold
      (fun () r st -> if st.cost = max_int then fprintf !pp "x%d " (P.to_int r))
      !costs ();
    fprintf !pp "\n\n");
...
let add_interfs_destroyed g live mregs =
  List.iter (fun mr -> VSet.iter (IRC.add_interf g (L (R mr))) live) mregs
...
let add_interfs_caller_save g live =
  VSet.iter
    (fun v ->
      ...
      List.iter
        (fun mr ->
          if not (is_callee_save mr && subtype tv (callee_save_type mr)) then
            IRC.add_interf g (L (R mr)) v)
        all_mregs)
    live

let add_interfs_live g live v = VSet.iter (fun v' -> IRC.add_interf g v v') live
let add_interfs_list g v vl = List.iter (IRC.add_interf g v) vl
...
let rec add_interfs_pairwise g = function
  | [] -> ()
  | v1 :: vl ->
      add_interfs_list g v1 vl;
      add_interfs_pairwise g vl
...
let add_interfs_instr g instr live =
  ...
  | Xparmove (srcs, dsts, itmp, ftmp) ->
      ...
      add_interfs_live g across itmp;
      add_interfs_live g across ftmp;
      ...
      add_interfs_list g itmp srcs;
      add_interfs_list g itmp dsts;
      add_interfs_list g ftmp srcs;
      add_interfs_list g ftmp dsts;
      (* Take into account destroyed reg when accessing Incoming param *)
      if
        List.exists
          (function L (Locations.S (Incoming, _, _)) -> true | _ -> false)
          srcs
      then (
        add_interfs_list g (vmreg temp_for_parent_frame) dsts;
        add_interfs_live g across (vmreg temp_for_parent_frame))
  | Xop (op, args, res) ->
      (match is_two_address op args with
      | None -> add_interfs_def g res live
      | Some (arg1, argl) ->
  ...
  | Xcall (sg, vos, args, res) ->
      ...
      | Coq_inl v ->
          List.iter
            (fun r -> IRC.add_interf g (vmreg r) v)
  ...
  | Xtailcall (sg, Coq_inl v, args) ->
      List.iter
        (fun r -> IRC.add_interf g (vmreg r) v)
        (int_callee_save_regs @ destroyed_at_indirect_call)
  ...
  | Xjumptable (arg, tbl) -> add_interfs_destroyed g live destroyed_by_jumptable
  | Xreturn optarg -> ()
...
let is_reg alloc v = match alloc v with R _ -> true | Locations.S _ -> false

let add_tospill alloc v ts =
  match alloc v with R _ -> ts | Locations.S _ -> VSet.add v ts

let addlist_tospill alloc vl ts = List.fold_right (add_tospill alloc) vl ts
...
let tospill_instr alloc instr ts =
  match instr with
  | Xmove (src, dst) ->
      if is_reg alloc src || is_reg alloc dst || alloc src = alloc dst then ts
      else VSet.add src (VSet.add dst ts)
  | Xreload (src, dst) ->
      if not (is_reg alloc dst) then (
        printf "Error: %a was spilled\n" PrintXTL.var dst;
        assert false);
      ts
  | Xspill (src, dst) ->
      if not (is_reg alloc src) then (
        printf "Error: %a was spilled\n" PrintXTL.var src;
        assert false);
      ts
  | Xparmove (srcs, dsts, itmp, ftmp) ->
      ...
      ts
  | Xop (op, args, res) -> addlist_tospill alloc args (add_tospill alloc res ts)
  | Xload (chunk, addr, args, dst) ->
      addlist_tospill alloc args (add_tospill alloc dst ts)
  | Xstore (chunk, addr, args, src) ->
      addlist_tospill alloc args (add_tospill alloc src ts)
  | Xcall (sg, vos, args, res) -> addros_tospill alloc vos ts
  | Xtailcall (sg, vos, args) -> addros_tospill alloc vos ts
  | Xbuiltin ((EF_annot _ | EF_debug _), _, _) -> ts
  | Xbuiltin (ef, args, res) ->
      addlist_tospill alloc
        (params_of_builtin_args args)
        (addlist_tospill alloc (params_of_builtin_res res) ts)
  | Xbranch s -> ts
  | Xcond (cond, args, s1, s2) -> addlist_tospill alloc args ts
  | Xjumptable (arg, tbl) -> add_tospill alloc arg ts
  | Xreturn optarg -> ts
...
let tospill_function f alloc =
  PTree.fold1 (fun ts blk -> tospill_block alloc blk ts) f.fn_code VSet.empty
...
let kill x eqs = List.filter (fun (v, t, date) -> v <> x && t <> x) eqs

let reload_var tospill eqs v =
  if not (VSet.mem v tospill) then (v, [], eqs)
  else
...
let save_var tospill eqs v =
  if not (VSet.mem v tospill) then (v, [], kill v eqs)
  else
    let t = new_temp (typeof v) in
...
let rec trim count eqs =
  if count <= 0 then []
    ...
    | [] -> []
    | (v, t, date) :: eqs' ->
        if date <= !max_age then (v, t, date + 1) :: trim (count - 1) eqs'
        else []
...
let spill_instr tospill eqs instr =
  ...
  match instr with
  | Xmove (src, dst) ->
      if VSet.mem src tospill && VSet.mem dst tospill then
        let src', c1, eqs1 = reload_var tospill eqs src in
        (c1 @ [ Xspill (src', dst) ], add dst src' (kill dst eqs1))
      else ([ instr ], kill dst eqs)
  | Xreload (src, dst) -> assert false
  | Xspill (src, dst) -> assert false
  | Xparmove (srcs, dsts, itmp, ftmp) ->
  ...
  | Xop (op, args, res) -> (
      ...
      | Some (arg1, argl) -> (
          match (VSet.mem res tospill, VSet.mem arg1 tospill) with
          | false, false ->
          ...
          | true, true ->
              ...
              let argl', c1, eqs2 = reload_vars tospill eqs1 argl in
              ( (Xreload (arg1, tmp) :: c1)
                @ [ Xop (op, tmp :: argl', tmp); Xspill (tmp, res) ],
  ...
  | Xcall (sg, Coq_inr id, args, res) -> ([ instr ], [])
  | Xtailcall (sg, Coq_inl v, args) ->
      let v', c1, eqs1 = reload_var tospill eqs v in
      (c1 @ [ Xtailcall (sg, Coq_inl v', args) ], [])
  | Xtailcall (sg, Coq_inr id, args) -> ([ instr ], [])
  | Xbuiltin ((EF_annot _ | EF_debug _), args, res) -> ([ instr ], eqs)
  | Xbuiltin (ef, args, res) ->
...
(** Apply a register allocation to an XTL function, producing an LTL function.
    Raise [Bad_LTL] if some pseudoregisters were mapped to stack locations while
    machine registers were expected, or in other words if spilling and reloading
    code must be inserted. *)

exception Bad_LTL

let mreg_of alloc v =
  match alloc v with R mr -> mr | Locations.S _ -> raise Bad_LTL
...
let make_move src dst k =
  ...
  | R rsrc, Locations.S (sl, ofs, ty) -> LTL.Lsetstack (rsrc, sl, ofs, ty) :: k
  | Locations.S (sl, ofs, ty), R rdst -> LTL.Lgetstack (sl, ofs, ty, rdst) :: k
  | Locations.S _, Locations.S _ -> if src = dst then k else raise Bad_LTL

type parmove_status = To_move | Being_moved | Moved

let make_parmove srcs dsts itmp ftmp k =
  let src = Array.of_list srcs and dst = Array.of_list dsts in
  let n = Array.length src in
  assert (Array.length dst = n);
  let status = Array.make n To_move in
  let temp_for cls =
    match cls with 0 -> itmp | 1 -> ftmp | _ -> assert false
  in
  let code = ref [] in
  let add_move s d =
    match (s, d) with
    | R rs, R rd -> code := LTL.Lop (Omove, [ rs ], rd) :: !code
    ...
    | Locations.S (sls, ofss, tys), Locations.S (sld, ofsd, tyd) ->
        ...
        code :=
          LTL.Lsetstack (tmp, sld, ofsd, tyd)
          :: LTL.Lgetstack (sls, ofss, tys, tmp)
          :: !code
  in
  let rec move_one i =
    if src.(i) <> dst.(i) then (
...
let transl_instr alloc instr k =
  ...
  | Xop (op, args, res) -> (
      let rargs = mregs_of alloc args and rres = mreg_of alloc res in
      match is_two_address op rargs with
      | None -> LTL.Lop (op, rargs, rres) :: k
      | Some (rarg1, rargl) ->
          if rarg1 = rres then LTL.Lop (op, rargs, rres) :: k
          else
            LTL.Lop (Omove, [ rarg1 ], rres)
            :: LTL.Lop (op, rres :: rargl, rres)
            :: k)
  | Xload (chunk, addr, args, dst) ->
  ...
  | Xcall (sg, vos, args, res) -> LTL.Lcall (sg, mros_of alloc vos) :: k
  | Xtailcall (sg, vos, args) -> LTL.Ltailcall (sg, mros_of alloc vos) :: []
  | Xbuiltin (ef, args, res) ->
      LTL.Lbuiltin
        ( ef,
          List.map (AST.map_builtin_arg alloc) args,
          AST.map_builtin_res (mreg_of alloc) res )
      :: k
  | Xbranch s -> LTL.Lbranch s :: []
  | Xcond (cond, args, s1, s2) ->
      LTL.Lcond (cond, mregs_of alloc args, s1, s2) :: []
  | Xjumptable (arg, tbl) -> LTL.Ljumptable (mreg_of alloc arg, tbl) :: []
  | Xreturn optarg -> LTL.Lreturn :: []
...
let rec first_round f liveness =
  ...
  if !option_dalloctrace then (
    ...
    PrintXTL.print_function !pp ~alloc ~live:liveness f);
...
and more_rounds f ts count =
  ...
  if !option_dalloctrace then (
    ...
    PrintXTL.print_function !pp ~alloc ~live:liveness f');
  let ts' = tospill_function f' alloc in
  if VSet.is_empty ts' then success f' alloc
  else (
    if !option_dalloctrace then (
...
let regalloc f =
  ...
  | Errors.Error msg ->
      Errors.Error
        (Errors.MSG
           (coqstring_of_camlstring "RTL code after splitting is ill-typed:")
        :: msg)
  | Errors.OK tyenv -> (
      ...
      try Errors.OK (first_round f3 liveness) with
      | Timeout ->
          Errors.Error
            (Errors.msg (coqstring_of_camlstring "spilling fails to converge"))
      | Type_error_at pc ->
          Errors.Error
            [
              Errors.MSG (coqstring_of_camlstring "ill-typed XTL code at PC ");
              Errors.POS pc;
            ]
      | Bad_LTL ->
          Errors.Error
            (Errors.msg (coqstring_of_camlstring "bad LTL after spilling")))
