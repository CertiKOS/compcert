...
module Linking : sig
  ...
  val get_globdef :
    t ->
    name:ident ->
    (string * (Clight.coq_function Ctypes.fundef, Ctypes.coq_type) AST.globdef)
    option

  val has_internal_sym : t -> mod_name:string -> name:ident -> bool

  val types_are_compat :
    t ->
    mod_1:string ->
    ty_1:composite_definition ->
    mod_2:string ->
    ty_2:composite_definition ->
    bool
  ...
  val add_globdef :
    ...
    name:ident ->
    mod_name:string ->
    defn:(Clight.coq_function Ctypes.fundef, Ctypes.coq_type) AST.globdef ->
    unit
  ...
  type t = {
    ...
    globvars :
      ( ident,
        string
        * (Clight.coq_function Ctypes.fundef, Ctypes.coq_type) AST.globdef )
      Hashtbl.t;
  ...
  let set_main_module state =
    Hashtbl.iter
      (fun id (m, _) ->
        if "main_inner" = ident_to_string ~name:id then
          state.main_module <- Some m)
      state.globvars;
    match state.main_module with
    | None -> Printf.printf "No MAIN MODULE found"
    | Some mm -> Printf.printf "MAIN MODULE FOUND AND IS: %s" mm
  ...
  let dump_rep_ty_table (state : t) unit =
    ...
    Hashtbl.iter
      (fun (m_1, ty_1) (m_2, ty_2) ->
        Printf.printf "\t module %s, ty %s (%ld) -> module %s, ty %s (%ld)\n"
          (ident_to_string ~name:ty_1)
          m_1 (ty_1 |> P.to_int32) m_2
          (ident_to_string ~name:ty_2)
          (ty_2 |> P.to_int32))
      state.r_ty_map;
  ...
  let dump_tylist (state : t) unit =
    ...
    Hashtbl.iter
      (fun (m, ty_uid) v ->
        Printf.printf "mod %s, ty %s (%ld)\n" m
          (ident_to_string ~name:ty_uid)
          (ty_uid |> P.to_int32))
      state.type_map;
    ...
    Hashtbl.iter
      (fun (m, ty_uid) s ->
        Printf.printf "\t (%s, %s (%ld)) -> {%s}\n" m
          (ident_to_string ~name:ty_uid)
          (ty_uid |> P.to_int32) (print_rep_set state s))
      state.rep_types;
  ...
  let add_globdef (state : t) ~name ~mod_name ~defn =
    ...
    else Hashtbl.replace state.globvars name (mod_name, defn)
  ...
  let has_internal_sym (state : t) ~mod_name ~name =
    ...
    match maybe_opt with Some tbl -> IdentSet.mem name tbl | None -> false
  ...
  let rec rep_ty_equal (state : t) ~mod_1 ~cty_1 ~mod_2 ~cty_2 =
    ...
    | Tstruct (i', a'), Tstruct (j', b') | Tunion (i', a'), Tunion (j', b') -> (
        if i' = j' then true
          ...
          match
            ( Hashtbl.find_opt state.r_ty_map (mod_1, i'),
              Hashtbl.find_opt state.r_ty_map (mod_2, j') )
          with
          | Some i'', Some j'' -> i'' = j''
          | _ -> false)
    | Tfunction (tl, rty, _cc), Tfunction (tl', rty', _cc') ->
        rep_tylist_equal state ~mod_1 tl ~mod_2 tl'
        && rep_ty_equal state ~mod_1 ~cty_1:rty ~mod_2 ~cty_2:rty'
        && _cc = _cc'
    | Tpointer (ty, _cc), Tpointer (ty', _cc') ->
        rep_ty_equal state ~mod_1 ~cty_1:ty ~mod_2 ~cty_2:ty'
    | Tarray (ty, num, _cc), Tarray (ty', num', _cc') ->
        rep_ty_equal state ~mod_1 ~cty_1:ty ~mod_2 ~cty_2:ty'
        && num = num' && _cc = _cc'
    | _ -> cty_1 = cty_2
  ...
  and rep_tylist_equal (state : t) ~mod_1 tl1 ~mod_2 tl2 =
    ...
    | Tcons (ty1, tl1'), Tcons (ty2, tl2') ->
        rep_ty_equal state ~mod_1 ~cty_1:ty1 ~mod_2 ~cty_2:ty2
        && rep_tylist_equal state ~mod_1 tl1' ~mod_2 tl2'
    | Tnil, Tnil -> true
    | _ -> false
        ...
        let mem_eq_fn =
         fun mems_1 mems_2 ->
          ...
          | Ctypes.Member_plain (id_1, ty), Ctypes.Member_plain (id_2, ty') ->
              ((is_anon_ident state ~name:id_1 && is_anon_ident state ~name:id_2)
              || id_1 = id_2)
  ...
  let rec internal_has_rep_type (state : t) (mod_1 : string)
      (ty_1 : composite_definition) (l : (string * composite_definition) list) =
    match l with
    | (mod_2, (Composite (id, _, _, _) as ty_2)) :: l' ->
        if types_are_compat state ~mod_1 ~ty_1 ~mod_2 ~ty_2 then Some (mod_2, id)
        else internal_has_rep_type state mod_1 ty_1 l'
    | nil -> None

  let has_rep_type (state : t) (mod_name : string) (cd : composite_definition) :
      tyuid option =
    let ty_list =
      Hashtbl.to_seq_keys state.rep_types
      |> List.of_seq
      |> List.map (fun x -> (fst x, Hashtbl.find state.type_map x))
    in
    internal_has_rep_type state mod_name cd ty_list
  ...
  let rec internal_fill_out_rep_types_initial (state : t)
      (l : (tyuid * composite_definition) list) =
    match l with
    | (((mod_name, id) as typ_uid), (Composite (_, sou, mems, attrs) as cd))
      :: l' ->
  ...
  let rec internal_check_rep_types (state : t) (tid : tyuid) (l : tyuid list) =
    ...
    | tid2 :: l' ->
        ...
        if
          types_are_compat state ~mod_1:(fst tid) ~mod_2:(fst tid2) ~ty_1:cd1
            ~ty_2:cd2
        then (
          ...
          TySet.iter
            (fun ele -> Hashtbl.replace state.r_ty_map ele tid2)
            tid_rep_eles;
          ...
          Printf.printf "TID1 %s %ld, TID2 is %s %ld \n"
            (ident_to_string ~name:(snd tid))
            (tid |> snd |> P.to_int32)
            (ident_to_string ~name:(snd tid2))
            (snd tid2 |> P.to_int32);

          flush stdout;

          let tid2_rep_eles = Hashtbl.find state.rep_types tid2 in
          Hashtbl.replace state.rep_types tid2
            (TySet.union tid_rep_eles tid2_rep_eles);
          true
  ...
  let rec internal_fill_out_rep_types_rest (state : t) (made_changes : bool)
      (l : tyuid list) =
    match l with
    | tid :: l' ->
  ...
  let rec get_fixed_point (state : t) =
    ...
    let made_changes =
      internal_fill_out_rep_types_rest state false list_of_tys
    in
...
module Imports : sig
  ...
  val get_globvars :
    t ->
    (ident * (RustLight.r_function Ctypes.fundef, Ctypes.coq_type) AST.globdef)
    list
  ...
  let get_globvars state =
    ...
    List.filter_map
      (fun (id, dfn) ->
        ...
        | AST.Gvar v ->
            if List.length v.gvar_init > 0 then Some (id, dfn) else None
        | _ ->
            if
              Hashtbl.mem gvs id
              || Linking.has_internal_sym state.linking ~mod_name:state.mod_name
                   ~name:id
            then Some (id, dfn)
            else None)
  ...
  let get_in_module_composite_defns state =
    ...
    |> List.map (fun id ->
           List.find
             (fun cd -> match cd with Composite (id', _, _, _) -> id = id')
    ...
    List.fold_left
      (fun acc
        ...
        | id, Gfun (Internal rf) ->
            ...
            dump_identset
              (Printf.sprintf "used idents from %s"
  ...
  let get_in_module_composite_typs (state : t) =
    List.iter
      (fun (Composite (id, _, _, _) as cd : composite_definition) ->
        let ty_id = (state.mod_name, id) in

        match Linking.get_rep_type_opt state.linking ~ty_id with
        | None ->
            Printf.printf "ERROR: can't find %s eg %ld in mod %s\n"
              (Linking.ident_to_string ~name:(snd ty_id))
              (P.to_int32 (snd ty_id))
              state.mod_name
        | Some rep_tyuid -> (
            if
            ...
            then
              state.in_module_composite_dfns <-
                IdentSet.add (snd ty_id) state.in_module_composite_dfns
            else
              ...
              let maybe_name =
                if rep_name = ty_name then None else Some ty_name
              in
              ...
              | None ->
                  Hashtbl.replace state.imports rep_mod
                    (ImportSet.singleton ele)))
      state.r_prog.prog_types
        ...
        (fun _ is acc ->
          ...
          || ImportSet.fold
               (fun (name, id') acc ->
                 ...
                 ||
                 match name with
                 | None -> Linking.ident_to_string ~name:id' = ident_name
                 | Some name -> name = ident_name)
               is false)
        state.imports false
    in
  ...
  let get_used_tys (state : t) (cd : composite_definition) =
    get_contained_typ_idents cd
    |> IdentSet.filter (fun id -> not (ident_already_exists state id))

  let set_extern_typs_from_in_module_composite_defns (state : t) =
    IdentSet.iter
      (fun ty_id ->
        let cd =
          Linking.get_type_definition state.linking
            ~ty_id:(state.mod_name, ty_id)
        in
      ...
      (fun m import_set ->
        ...
        ImportSet.iter
          (fun (alias_opt, id) ->
            ...
            | Some alias -> Printf.printf "\t\t%s as %s\n" id_str alias
      ...
      (fun id ->
        ...
        | Some (m, defn) -> (
            if m = state.mod_name |> not then
              ...
              | None ->
                  Hashtbl.replace state.imports m (ImportSet.singleton ele))
        | None ->
            Printf.printf "PROCCESSING %s %ld"
              (Linking.ident_to_string ~name:id)
              (id |> P.to_int32);

            (* if the symbol doesn't exist, check if it's statically linked in the file. If it is: do nothing. *)
            if
              Linking.has_internal_sym state.linking ~name:id
  ...
  let check_if_type_is_elsewhere (state : t) (inner_ident : ident)
      (id_fn : ident) =
    Printf.printf "\nUID Considering %s from module %s for function %s\n"
      (Linking.ident_to_string ~name:inner_ident)
      state.mod_name
      (Linking.ident_to_string ~name:id_fn);
    match Linking.get_globdef state.linking ~name:id_fn with
    | None -> Printf.printf "\t couldn't find\n"
    | Some (mod_name, _) -> (
        Printf.printf "\t found in module %s\n" mod_name;
        match
          Linking.get_rep_type_opt state.linking ~ty_id:(mod_name, inner_ident)
        with
        | None -> Printf.printf "\t couldn't find rep type\n"
        | Some (rep_mod_name, rep_ty) ->
            ...
            let rename =
              ...
              then None
              else Some (Linking.ident_to_string ~name:inner_ident)
            in
            (match Hashtbl.find_opt state.imports rep_mod_name with
            | Some is ->
                Hashtbl.replace state.imports rep_mod_name
                  (ImportSet.add (rename, rep_ty) is)
            | None ->
                Hashtbl.replace state.imports rep_mod_name
                  (ImportSet.singleton (rename, rep_ty)));
  ...
  let rec check_typ (state : t) (ty : Ctypes.coq_type) (id_fn : ident) =
    ...
    | Tpointer (ty', _) | Tarray (ty', _, _) -> check_typ state ty' id_fn
    | Tfunction (_, _, _) -> ()
    | _ -> ()
  ...
  let rec fixup_fn (state : t)
      (prog_def : AST.ident * ('f Ctypes.fundef, Ctypes.coq_type) AST.globdef) =
    match prog_def with
    | id, Gfun (External (ef, tl, rty, _)) -> fixup_tylist state tl id
    | _, _ -> ()
