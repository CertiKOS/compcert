...
module Linking : sig
  ...
  (* ty_id -> (mod_name, defn)*)
  val get_type_definition: t -> ty_id: ident -> (string * composite_definition)

  val get_rep_type_definition: t -> ty_id: ident -> (string * composite_definition)

  val types_are_compat: t -> ty_1: composite_definition -> ty_2: composite_definition -> bool
  ...
  struct
    ...
    let dump_rep_ty_table (state: t) unit =
      ...
      Hashtbl.iter (fun k v -> Printf.printf "\t %s (%ld) -> %s (%ld)\n" (ident_to_string state ~name:k) (k |> P.to_int32) (ident_to_string state ~name:v) (v |> P.to_int32)) state.r_ty_map;
      Printf.printf "Ending dump r_ty_map: \n";
      flush stdout

    let dump_tylist (state: t) unit =
      Printf.printf "Begininning dump type_map: \n";
      Hashtbl.iter (fun k v -> Printf.printf "%s (%ld)\n" (ident_to_string state ~name:k) (k |> P.to_int32)) state.type_map;
      Printf.printf "Ending dump type_map: \n";
      flush stdout

    let print_rep_set s =
      IdentSet.fold (fun s acc -> Printf.sprintf "%s %ld," acc (s |> P.to_int32)) s ""

    let dump_rep_types (state: t) =
      Printf.printf "Begininning dump rep_types: \n";
      Hashtbl.iter (fun k s ->
        Printf.printf "\t %s (%ld) -> {%s}\n" (ident_to_string state ~name:k) (k |> P.to_int32) (print_rep_set s)
    ) state.rep_types;
    ...
    let add_ty_defn (state: t) ~mod_name ~cd =
      ...
      Hashtbl.add state.type_map id (mod_name, cd)
    ...
    let rec rep_ty_equal (state: t) ~cty_1 ~cty_2 =
      match (cty_1, cty_2) with
      | (Tstruct(i', a'), Tstruct(j', b'))
      | (Tunion(i', a'), Tunion(j', b')) ->(
          if i' = j' then true
          else
            match (Hashtbl.find_opt state.r_ty_map i', Hashtbl.find_opt state.r_ty_map j') with
            | (Some(i''), Some(j'')) -> i'' = j''
            | _ -> false)
      | (Tfunction(tl, rty, _cc), Tfunction(tl', rty', _cc')) ->
          rep_tylist_equal state tl tl' && rep_ty_equal state ~cty_1:rty ~cty_2:rty' && _cc = _cc'
      | (Tpointer(ty, _cc), Tpointer(ty', _cc')) ->
          rep_ty_equal state ~cty_1:ty ~cty_2:ty'
      | (Tarray(ty, num, _cc), Tarray(ty', num', _cc')) -> rep_ty_equal state ~cty_1:ty ~cty_2:ty' && num = num' && _cc = _cc'
      | _ -> cty_1 = cty_2
    and rep_tylist_equal (state: t) tl1 tl2 =
      match (tl1, tl2) with
      | (Tcons(ty1, tl1'), Tcons(ty2, tl2')) ->
          rep_ty_equal state ~cty_1:ty1 ~cty_2:ty2 && rep_tylist_equal state tl1' tl2'
      | (Tnil, Tnil) -> true
      | _ -> false

    let types_are_compat (state: t) ~ty_1 ~ty_2 =
      ...
      | Ctypes.Composite (_, sou, mems, attrs), Ctypes.Composite(_, sou_, mems_, attrs_) -> (
          ...
          let mem_eq_fn = fun m_1 m_2 -> (
              match (m_1, m_2) with
              | Ctypes.Member_plain(_, ty), Ctypes.Member_plain(_, ty') -> rep_ty_equal state ~cty_1:ty ~cty_2:ty'
    ...
    let get_rep_type_definition (state: t) ~ty_id =
      get_type_definition state ~ty_id:(Hashtbl.find state.r_ty_map ty_id )

    (* TODO the way to handle mutual recursion is a stack. But, we don't yet handle that *)
    (* need to make a test case then handle it. *)
    let rec internal_has_rep_type (state: t) (ty_1: composite_definition) (l: composite_definition list) =
      match l with
      | (Composite(id, _, _, _) as ty_2) :: l' ->
          if types_are_compat state ~ty_1 ~ty_2 then
            Some(id)
          else internal_has_rep_type state ty_1 l'
      | nil -> None


    let has_rep_type (state: t) (cd: composite_definition) =
      let ty_list = Hashtbl.to_seq_keys state.rep_types |> List.of_seq |> List.map (fun x -> Hashtbl.find state.type_map x |> snd)
      in internal_has_rep_type state cd ty_list

    (* if it's contained: great *)
    (* if it's not contained: add as a representative type *)
    let rec internal_fill_out_rep_types_initial (state: t) (l: (ident * (string * composite_definition))list) =
      match l with
      | (id, (mod_name, Composite(_, sou, mems, attrs) as cd)) :: l' ->
          (match has_rep_type state (snd cd) with
          | None -> (
            Hashtbl.add state.rep_types id (IdentSet.singleton id);
            Hashtbl.add state.r_ty_map id id;
          )
          | Some(rep_id) -> (
            let old_set = Hashtbl.find state.rep_types rep_id in
            let new_set = IdentSet.add id old_set in
            Hashtbl.replace state.rep_types rep_id new_set;
            Hashtbl.replace state.r_ty_map id rep_id;
          ));
          internal_fill_out_rep_types_initial state l'
      | nil -> ()

    let rec internal_check_rep_types (state: t) (tid: ident) (l: ident list) =
      dump_rep_ty_table state ();
      dump_rep_types state;

      match l with
      | tid2 :: l' -> (
          let cd1 = Hashtbl.find state.type_map tid |> snd in
          let cd2 = Hashtbl.find state.type_map tid2 |> snd in
          if types_are_compat state ~ty_1:cd1 ~ty_2:cd2 then(
            ...
            dump_rep_types state;
            Printf.printf "TID1 %s %ld, TID2 is %s %ld \n" (ident_to_string state ~name:tid) (tid |> P.to_int32) (ident_to_string state ~name:tid2) (tid2 |> P.to_int32);

            flush stdout;

            let tid2_rep_eles = Hashtbl.find state.rep_types tid2 in
            Hashtbl.replace state.rep_types tid2 (IdentSet.union tid_rep_eles tid2_rep_eles);
            true
    ...
    let rec internal_fill_out_rep_types_rest (state: t) (made_changes: bool) (l: ident list) =
      match l with
      | tid :: l' ->
