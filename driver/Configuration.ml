...
let search_argv key =
  ...
  for i = 1 to len - 2 do
    if Commandline.argv.(i) = key then res := Some Commandline.argv.(i + 1)
  done;
  !res

let absolute_path base file =
  if Filename.is_relative file then Filename.concat base file else file
...
let ini_file_name =
  ...
  | None -> (
      try Sys.getenv "COMPCERT_CONFIG"
      with Not_found -> (
        let ini_name =
          match search_argv "-target" with
          | Some s -> s ^ ".ini"
          | None -> "compcert.ini"
        in
        ...
        let share_dir =
          Filename.concat
            (Filename.concat exe_dir Filename.parent_dir_name)
            "share"
        in
        let share_compcert_dir = Filename.concat share_dir "compcert" in
        let search_path = [ exe_dir; share_dir; share_compcert_dir ] in
        let files =
          List.map (fun s -> Filename.concat s ini_name) search_path
        in
...
let _ =
  try Readconfig.read_config_file ini_file_name with
...
let get_config key =
  ...
  | Some v -> v
  | None ->
      eprintf "Configuration option `%s' is not set\n" key;
      exit 2
...
let get_config_string key =
  match get_config key with [ v ] -> v | vl -> bad_config key vl

let get_config_list key =
  match get_config key with [] -> bad_config key [] | vl -> vl
...
let tool_absolute_path tools =
  ...
  | tool :: args ->
      let tool =
        if
          Filename.is_implicit tool
          && Filename.dirname tool = Filename.current_dir_name
        then tool
        else absolute_path ini_dir tool
      in
      tool :: args

let opt_config_list key =
  match Readconfig.key_val key with Some v -> v | None -> []

let prepro = tool_absolute_path (get_config_list "prepro" @ opt_config_list "prepro_options")
let asm = tool_absolute_path (get_config_list "asm" @ opt_config_list "asm_options")
let linker = tool_absolute_path (get_config_list "linker" @ opt_config_list "linker_options")
