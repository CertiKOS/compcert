...
let transform_program ~unblock ~switch_norm ~struct_passing ~packed_structs p =
  let run_pass pass p =
    ...
    p'
  in
  let run_opt_pass pass flag p = if flag then run_pass pass p else p
  and run_opt_pass3 pass flag p =
    match flag with
    | `Off -> p
    | `Partial -> run_pass (pass false) p
    | `Full -> run_pass (pass true) p
  in
...
let parse_string name text =
  ...
  | _ ->
      (* Fail_pr or Fail_pr_full or Timeout_pr, depending
...
let preprocessed_file ?(unblock = false) ?(switch_norm = `Off)
    ?(struct_passing = false) ?(packed_structs = false) name sourcefile =
  Diagnostics.reset ();
  let check_errors x =
    Diagnostics.check_errors ();
    x
...
let preprocessed_file_from_string ?(unblock = false) ?(switch_norm = `Off)
    ?(struct_passing = false) ?(packed_structs = false) name source =
  Diagnostics.reset ();
  let check_errors x =
    Diagnostics.check_errors ();
    x
